...
# -----------------------------
# Random Search: Hyperparameters & Rewards
# -----------------------------
...
def smooth(y, box_pts=50):
    """
    Moving-average smoothing for 1D arrays.

    Args:
        y (array-like): Input sequence.
        box_pts (int): Window size for the moving average.

    Returns:
        np.ndarray: Smoothed sequence (same length, 'same' convolution).
    """
...
def patch_env(cfg):
    """
    Monkey-patch AsteroidsEnv to inject config values and override _reward.

    Notes:
        - Keeps original __init__ (calls then sets attributes from cfg).
        - Replaces _reward with a custom function that reads tunable attributes.
        - Logic follows the baseline env._reward structure but uses cfg values.
    """
    original_init = AsteroidsEnv.__init__

    def new_init(self, render_mode=False):
        # Call original init to set up environment
        original_init(self, render_mode=render_mode)
        # Inject each configuration key/value as an attribute
    ...
    def custom_reward(self, shoot, safe_normalize):
        """
        Reward function that reads reward/penalty magnitudes from env attributes.
        """
        reward = 0
        alignment_reward = 0
        shooting_reward = 0
        ...
        # --- Edge penalties and termination ---
...
def sample_hyperparams(space, n):
    """
    Sample n configurations from the given hyperparameter space.

    Args:
        space (dict): key -> list of possible values.
...
for idx, cfg in enumerate(configs):
    # Inject config into environment behaviour
    patch_env(cfg)
    ...
    # Print config details (emojis removed for clarity/logging hygiene)
    print(f"\nConfig {idx}")
    for k, v in cfg.items():
        print(f"{k:<25} = {v}")
    print("-" * 55)

    # Run multiple training runs for this config
    all_rewards = []
...
# Right: final-episode performance (mean of last 100 eps)
